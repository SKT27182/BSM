...
* M2 incl. rad. corrs.:
     ...
     .     -8d0*NMB0(M22,M22,MW2,Q2)+4d0*NMB1(M22,M22,MW2,Q2)))
      ...
      IF(MZ.LE.2*MNEU(1))THEN
            ZXX= 0d0
           ELSE
            RH= MW*NEU(1,2)**2-MZ*NEU(1,4)**2+MZ*NEU(1,3)**2
    
            CH= SQRT(MZ**2-4*MNEU(1)**2)*(MZ**2+2*MNEU(1)**2)
            ZXX= SQRT(2d0)*GF/3d0/PI/MZ**2*CH*RH
           ENDIF
      END