...
*      PROB(73) =/= 0 BR_bsm(H_SM) 2 sigma away from LHC measured value
     ...
     .      BRSUSY,WIDTH
      ...
      CMIN(1)=0.966d0 ! electroweak couplings
      CMAX(1)=1.000d0 
      CMIN(2)=0.827d0 ! top quark
      CMAX(2)=1.131d0
      CMIN(3)=0.772d0 ! b quark
      CMAX(3)=1.081d0
      CMIN(4)=0.858d0 ! gluons
      CMAX(4)=1.060d0
      CMIN(5)=0.933d0 ! photons
      CMAX(5)=1.100d0
      CMIN(6)=0.842d0 ! taus
      CMAX(6)=1.046d0
...
*     PROB(75) =/= 0: excluded 
*     H_125 -> ZA, A -> muons from CMS, arXiv:2111.01299
*     PROB(76) =/= 0: excluded 

      IMPLICIT NONE

      INTEGER I,N1,N2,N3,N4
      PARAMETER (N1=9,N2=16,N3=129,N4=223)

      DOUBLE PRECISION PROB(*),S,SMAX,M1(N1),L1(N1),M2(N2),L2(N2)
      DOUBLE PRECISION M3(N3),L3(N3),M4(N4),L4(N4)
      ...
      DO I=1,2
       ...
       IF(SIGJ.NE.0d0)THEN
        ...
        IF(J.GE.2 .AND. MA.LE.M1(N1)) THEN
         ...
         PROB(78)=PROB(78)+DDIM(SIGJ/L,1d0)
        ENDIF
        ...
        IF(J.GE.2 .AND. MA.LE.M2(N2)) THEN
         ...
         PROB(78)=PROB(78)+DDIM(SIGB/L,1d0)
        ENDIF
        ...
        IF(J.GE.2 .AND. MA.LE.M3(N3)) THEN
         ...
         PROB(79)=PROB(79)+DDIM(SIGJ/L,1d0)
        ENDIF
        ...
        IF(J.GE.2 .AND. MA.LE.M3(N3)) THEN
         ...
         PROB(79)=PROB(79)+DDIM(SIGB/L,1d0)
        ENDIF
        ...
        IF(J.GE.2 .AND. MA.LE.M4(N4)) THEN
         ...
         PROB(79)=PROB(79)+DDIM(SIGJ/.569d0/L,1d0)
        ENDIF
...
*     PROB(84) =/= 0: excluded 
*     H -> HSM+HSM from ATLAS-CONF-2021-052
*     PROB(85) =/= 0: excluded 
*     H -> HSM+HSM from CMS arXiv:2112.03161
*     PROB(86) =/= 0: excluded 

      IMPLICIT NONE

      INTEGER I,J,K,N1,N2,N3
      PARAMETER (N1=22,N2=29,N3=38)
     ...
     .7.48d-3,5.34d-3,3.29d-3,2.82d-3,1.98d-3,1.16d-3/
      ...
      DATA M3/0.80d3,0.90d3,0.10d4,0.11d4,0.12d4,0.13d4,0.14d4,0.15d4,
     .0.16d4,0.17d4,0.18d4,0.19d4,0.20d4,0.21d4,0.22d4,0.23d4,0.24d4,
     .0.25d4,0.26d4,0.27d4,0.28d4,0.29d4,0.30d4,0.31d4,0.32d4,0.33d4,
     .0.34d4,0.35d4,0.36d4,0.37d4,0.38d4,0.39d4,0.40d4,0.41d4,0.42d4,
     .0.43d4,0.44d4,0.45d4/
      DATA L3/0.24528d-1,0.95925d-2,0.47897d-2,0.34120d-2,0.26731d-2,
     .0.27118d-2,0.40980d-2,0.51152d-2,0.33314d-2,0.18524d-2,0.14377d-2,
     .0.13988d-2,0.15003d-2,0.16487d-2,0.17332d-2,0.18388d-2,0.19041d-2,
     .0.18471d-2,0.16354d-2,0.14237d-2,0.12476d-2,0.10629d-2,0.92380d-3,
     .0.87063d-3,0.86477d-3,0.87187d-3,0.87752d-3,0.87476d-3,0.86268d-3,
     .0.84510d-3,0.82539d-3,0.80585d-3,0.79000d-3,0.78436d-3,0.78830d-3,
     .0.79250d-3,0.79002d-3,0.78119d-3/
     ...
     .             +DDIM(SIG*BRBB(J)/5.824d-1*BRGG(J)/2.270d-3/L,1d0)
          ...
          IF(K.GE.2 .AND. MH.LE.M2(N2)) THEN
           ...
           PROB(85)=PROB(85)+DDIM(SIG/L,1d0)
          ENDIF
          ...
          IF(K.GE.2 .AND. MH.LE.M3(N3)) THEN
           ...
           PROB(86)=PROB(86)+DDIM(SIG/L,1d0)
          ENDIF
         ENDIF
        ENDIF
       ENDDO
