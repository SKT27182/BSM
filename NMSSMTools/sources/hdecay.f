...
*      HILLLR(i)       coupling ai,~e_L,~e_R (i=1,2, 3rd family only)
      ...
      DOUBLE PRECISION ZHA, BRMM1, BRMM4, BRHAA21, BRHAZ21
...
!        DO J=1,2
!         IF(MZ.LE.PMASS(J)+SMASS(1))THEN
!          ZHA(J)= 0d0
!         ELSE
!          RH= (S(I,1)*COSBETA-S(I,2)*SINBETA)
!      .     * (P(J,1)*COSBETA+P(J,2)*SINBETA)
!           CH = LAMB((PMASS(1)/MZ)**2, (SMASS(1)/MZ)**2) 
!      .     * (MZ**4 + 4*SMASS(1)**2*PMASS(1)**2 
!      .     - (MZ**2 - SMASS(1)**2 - PMASS(1)**2)**2)
!          ZHA(J)= GF/12d0/SQR2/PI*MZ**2*CH*RH**2
!          ZTOT= ZTOT+ZHA(J)
!         ENDIF
!        ENDDO
...
!        IF(MZ.LE.PMASS(1)+SMASS(1))THEN
!         ZHA= 0d0
!        ELSE
!         RH= (S(1,1)*P(1,1)-S(1,2)*P(1,2))
!         CH= LAMB((PMASS(1)/MZ)*2, (SMASS(1)/MZ)*2)
!      .    * MZ*4 + 4((SMASS(1))*2)*PMASS(1)*2
...
*   z -> ah
     ...
     .    *  MZ**4 + 4*((SMASS(1))**2)*PMASS(1)**2 
     .     - (MZ**2 - (SMASS(1))**2 - PMASS(1)**2)**2
         ZHA= GF/12d0/SQR2/PI/MZ**1*CH*RH**2
       ENDIF

*     h2-> invisible
       BRINV1=BRNEU(2,1,1)
     .   +BRHAA(2,1)*BRNEU(4,1,1)**2
     . +BRHAA(2,2)*BRNEU(4,1,1)*BRNEU(5,1,1)
     . +BRHAA(2,3)*BRNEU(5,1,1)**2
