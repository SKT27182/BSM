...
************************************************************************
      ...
      DOUBLE PRECISION ZTOT, ZHA, BRMM1, BRMM4, BRHAA21, BRHAZ21
      COMMON/ZSPEC/ZHA, BRMM1, BRMM4, BRHAA21, BRHAZ21
      DOUBLE PRECISION BRLL(5), BRBB(5), BREE(5), BRGG(5), BRHHH(4)
      COMMON/BRN/BRLL,BRBB,BREE, BRGG, BRHHH
      double precision ZXX
      COMMON/ZDECAY/ZXX


      DOUBLE PRECISION BRZEE, BRZMUMU, BRZTAUTAU, BRZUUB, BRZDDB, BRZSSB
      DOUBLE PRECISION BRZCCB, BRZBBB, BRZHA, BRMUMUMUMU, BRZBBBB

      DOUBLE PRECISION SMASS(3),S(3,3)
      DOUBLE PRECISION PMASS(2),P2(2,2),CMASS
...
c --------- c
      ...
      ZTOT = 2.4952d0+ZHA+ZXX
      BRZEE = 0.03365d0*2.4952d0/(ZTOT)
      BRZMUMU = 0.03365d0*2.4952d0/(ZTOT)
      BRZTAUTAU = 0.0337d0*2.4952d0/(ZTOT)
      BRZUUB = 0.111d0*2.4952d0/(ZTOT)
      BRZDDB = 0.1585d0*2.4952d0/(ZTOT)
      BRZSSB = 0.111d0*2.4952d0/(ZTOT)
      BRZCCB = 0.12d0*2.4952d0/(ZTOT)
      BRZBBB = 0.151d0*2.4952d0/(ZTOT)
      BRZHA = ZHA/(ZTOT)
      BRZBBBB = ZHA*BRMM1*BRMM4/(ZTOT)
      BRMUMUMUMU = ZHA*BRMM1*BRMM4/(ZTOT)
...
c --------- c
      ...
      write(nout,99)
      write(nout,69) iz,ZTOT,'Z (measured)'
      write(nout,102) ZXX/(ZTOT) ,2,in1,in1,
     .'BR(Z -> XX)'
      write(nout,102) ((0.200d0*2.4952d0)+ZXX)/(ZTOT),2,-ine,ine,
     .'BR(Z -> invisible)'
      write(nout,102) BRZEE,2,-ie,ie,
     .'BR(Z -> e+ e-)'
      write(nout,102) BRZMUMU,2,-imu,imu,
     .'BR(Z -> mu+ mu-)'
      write(nout,102) BRZTAUTAU,2,-itau,itau,
     .'BR(Z -> tau+ tau-)'
      write(nout,102) BRZUUB,2,iu,-iu,
     .'BR(Z -> u ub)'
      write(nout,102) BRZDDB,2,id,-id,
     .'BR(Z -> d db)'
      write(nout,102) BRZSSB,2,is,-is,
     .'BR(Z -> s sb)'
      write(nout,102) BRZCCB,2,ic,-ic,
     .'BR(Z -> c cb)'
      write(nout,102) BRZBBB,2,ib,-ib,
     .'BR(Z -> b bb)'
      write(nout,102) BRZHA,2,ihH2,ihA1,
     .'BR(Z -> h a)'


!       write(nout,102) ZHA*BRMM1*BRMM4,2,ihA1,ihA1,
!      .'Decay Rate(Z -> mu+ mu- mu+ mu-)'
!       write(nout,102) BRMUMUMUMU,2,ihA1,ihA1, 
!      .'BR(Z -> mu+ mu- mu+ mu-)'
!       write(nout,102) BRZBBBB,2,ihA1,ihA1, 
!      .'BR(Z -> b b~ b b~)'
!       write(nout,102) ZHA*(BRGG(1)*(BREE(4)+BRMM4+BRLL(4)) 
!      .+ BRGG(4)*(BREE(1)+BRMM1+BRLL(1)))/(ZTOT),2,ihA1,ihA1, 
!      .'BR(Z -> l l~ gamma gamma)'
!       write(nout,102) (BRHHH(1)*BRBB(1)*BRBB(1))
!      ./(ZTOT),2,ihA1,ihA1, 
!       write(nout,102) BRHHH(1),2,ihA1,ihA1, 
!      .'BR(h2 -> h1 h1 -> b b~ b b~)'
!       write(nout,102) (BRHAA21*BRBB(4)*BRBB(4))
!      ./(ZTOT),2,ihA1,ihA1, 
!      .'BR(h2 -> a1 a1 -> b b~ b b~)'
!       write(nout,102) (BRHAZ21*BRBB(4)*BRZBBB)
!      ./(ZTOT),2,ihA1,ihA1, 
!      .'BR(h2 -> a1 z -> b b~ b b~)'
      
!       write(nout,105) (BRHAZ21*(BREE(4)+BRMM4+BRLL)*
!      .(BRZEE + BRZMUMU + BRZTAUTAU))/(ZTOT),2,ihA1,ihA1, 
!      .'BR(h2 -> a1 z -> l l~ l l~)'
!       write(nout,102) (BRHAZ21*((BREE(4)+BRMM4+BRLL)*(BRZBBB) + BRBB(4)
!      .* (BRZEE + BRZMUMU + BRZTAUTAU)))/(ZTOT),2,ihA1,ihA1, 
!      .'BR(h2 -> a1 z -> l l~ b b~)'
...
c ==================================================================== c
 ...
 105  format(3x,1P,E16.8,0P,3x,F6.6,3x,(F6.6,1x),(F6.6,1x),2x,'#',1x,A)
 69   format('DECAY',1x,I9,3x,1P,E20.10,0P,3x,'#',1x,A)
