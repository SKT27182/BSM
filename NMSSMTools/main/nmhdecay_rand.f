...
************************************************************************
     ...
     . NAU3,NAD3,NAE3,NAE2,NML3,NML2,NME3,
     . NME2,NMQ3,NMQ2,NMU3,NMU2,NMD3,NMD2
...
*   Beginning of the scan
      ...
      IF(AU3MIN.EQ.AU3MAX)THEN
       PAR(12)=AU3MIN
      ELSE
       IF(NAU3.EQ.0)THEN
        PAR(12)=AU3MIN+(AU3MAX-AU3MIN)*RAN2(IDUM)
       ELSE
        PAR(12)=AU3MIN*(AU3MAX/AU3MIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(AD3MIN.EQ.AD3MAX)THEN
       PAR(13)=AD3MIN
      ELSE
       IF(NAD3.EQ.0)THEN
        PAR(13)=AD3MIN+(AD3MAX-AD3MIN)*RAN2(IDUM)
       ELSE
        PAR(13)=AD3MIN*(AD3MAX/AD3MIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(AE3MIN.EQ.AE3MAX)THEN
       PAR(14)=AE3MIN
      ELSE
       IF(NAE3.EQ.0)THEN
        PAR(14)=AE3MIN+(AE3MAX-AE3MIN)*RAN2(IDUM)
       ELSE
        PAR(14)=AE3MIN*(AE3MAX/AE3MIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(AE2MIN.EQ.AE2MAX)THEN
       PAR(25)=AE2MIN
      ELSE
       IF(NAE2.EQ.0)THEN
        PAR(25)=AE2MIN+(AE2MAX-AE2MIN)*RAN2(IDUM)
       ELSE
        PAR(25)=AE2MIN*(AE2MAX/AE2MIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(ML3MIN.EQ.ML3MAX)THEN
       PAR(10)=ML3MIN**2
      ELSE
       IF(NML3.EQ.0)THEN
        PAR(10)=(ML3MIN+(ML3MAX-ML3MIN)*RAN2(IDUM))**2
       ELSE
        PAR(10)=(ML3MIN*(ML3MAX/ML3MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(ML2MIN.EQ.ML2MAX)THEN
       PAR(18)=ML2MIN**2
      ELSE
       IF(NML2.EQ.0)THEN
        PAR(18)=(ML2MIN+(ML2MAX-ML2MIN)*RAN2(IDUM))**2
       ELSE
        PAR(18)=(ML2MIN*(ML2MAX/ML2MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(ME3MIN.EQ.ME3MAX)THEN
       PAR(11)=ME3MIN**2
      ELSE
       IF(NME3.EQ.0)THEN
        PAR(11)=(ME3MIN+(ME3MAX-ME3MIN)*RAN2(IDUM))**2
       ELSE
        PAR(11)=(ME3MIN*(ME3MAX/ME3MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(ME2MIN.EQ.ME2MAX)THEN
       PAR(19)=ME2MIN**2
      ELSE
       IF(NME2.EQ.0)THEN
        PAR(19)=(ME2MIN+(ME2MAX-ME2MIN)*RAN2(IDUM))**2
       ELSE
        PAR(19)=(ME2MIN*(ME2MAX/ME2MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(MQ3MIN.EQ.MQ3MAX)THEN
       PAR(7)=MQ3MIN**2
      ELSE
       IF(NMQ3.EQ.0)THEN
        PAR(7)=(MQ3MIN+(MQ3MAX-MQ3MIN)*RAN2(IDUM))**2
       ELSE
        PAR(7)=(MQ3MIN*(MQ3MAX/MQ3MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(MQ2MIN.EQ.MQ2MAX)THEN
       PAR(15)=MQ2MIN**2
      ELSE
       IF(NMQ2.EQ.0)THEN
        PAR(15)=(MQ2MIN+(MQ2MAX-MQ2MIN)*RAN2(IDUM))**2
       ELSE
        PAR(15)=(MQ2MIN*(MQ2MAX/MQ2MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(MU3MIN.EQ.MU3MAX)THEN
       PAR(8)=MU3MIN**2
      ELSE
       IF(NMU3.EQ.0)THEN
        PAR(8)=(MU3MIN+(MU3MAX-MU3MIN)*RAN2(IDUM))**2
       ELSE
        PAR(8)=(MU3MIN*(MU3MAX/MU3MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(MU2MIN.EQ.MU2MAX)THEN
       PAR(16)=MU2MIN**2
      ELSE
       IF(NMU2.EQ.0)THEN
        PAR(16)=(MU2MIN+(MU2MAX-MU2MIN)*RAN2(IDUM))**2
       ELSE
        PAR(16)=(MU2MIN*(MU2MAX/MU2MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(MD3MIN.EQ.MD3MAX)THEN
       PAR(9)=MD3MIN**2
      ELSE
       IF(NMD3.EQ.0)THEN
        PAR(9)=(MD3MIN+(MD3MAX-MD3MIN)*RAN2(IDUM))**2
       ELSE
        PAR(9)=(MD3MIN*(MD3MAX/MD3MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(MD2MIN.EQ.MD2MAX)THEN
       PAR(17)=MD2MIN**2
      ELSE
       IF(NMD2.EQ.0)THEN
        PAR(17)=(MD2MIN+(MD2MAX-MD2MIN)*RAN2(IDUM))**2
       ELSE
        PAR(17)=(MD2MIN*(MD2MAX/MD2MIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF
...
*   Total number of points
     ...
     .(NMP.NE.0 .AND. NMP.NE.1) .OR. (NAU3.NE.0 .AND. NAU3.NE.1) .OR.
     .(NAD3.NE.0 .AND. NAD3.NE.1) .OR. (NAE3.NE.0 .AND. NAE3.NE.1) .OR.
     .(NAE2.NE.0 .AND. NAE2.NE.1) .OR. (NML3.NE.0 .AND. NML3.NE.1) .OR.
     .(NML2.NE.0 .AND. NML2.NE.1) .OR. (NME3.NE.0 .AND. NME3.NE.1) .OR.
     .(NME2.NE.0 .AND. NME2.NE.1) .OR. (NMQ3.NE.0 .AND. NMQ3.NE.1) .OR.
     .(NMQ2.NE.0 .AND. NMQ2.NE.1) .OR. (NMU3.NE.0 .AND. NMU3.NE.1) .OR.
     .(NMU2.NE.0 .AND. NMU2.NE.1) .OR. (NMD3.NE.0 .AND. NMD3.NE.1) .OR.
     .(NMD2.NE.0 .AND. NMD2.NE.1))THEN
