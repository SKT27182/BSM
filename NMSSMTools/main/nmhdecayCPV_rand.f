...
************************************************************************
      ...
      PARAMETER (NFL=10,NPROB=88,NPAR=25)
      ...
      INTEGER NMUPI,NMSPI,NM3HI,CFLAG(6)
...
*   Computation of parameters at QSTSB
      ...
      IF(GMUFLAG.NE.0)CALL magnmu_CPV(PROB)
...
*********************************************************************
      ...
      INTEGER I,S,TOT,NTOT,NFAIL(*),GMUFLAG,HFLAG,CFLAG(6)
      INTEGER M1FLAG,M2FLAG,M3FLAG,MHDFLAG,MHUFLAG,MSFLAG
      INTEGER AKFLAG,ALFLAG,XISIFLAG,OMGFLAG,MAFLAG,MOFLAG
     ...
     . MSFLAG,AKFLAG,ALFLAG,XISIFLAG
      ...
      WRITE(17,10)"Version number: 6.0.0         "
      WRITE(17,*)
      WRITE(17,*)
      WRITE(17,10)"Number of points:             "
      WRITE(17,*)
      WRITE(17,10)"  scanned                     ",NTOT
      WRITE(17,10)"  mu = 0 | k=0 & (Ak|XiS)=/=0 ",NFAIL(9)
      S=0
      DO I=1,7
       S=S+NFAIL(I)
      ENDDO
      WRITE(17,10)"  with mh1^2 or mhc^2 < 0     ",S
     ...
     .  WRITE(17,20)" - (g-2)_muon                      "
      ...
      IF(TOT.GT.0)THEN
       ...
       IF(M1FLAG.NE.0)THEN
        WRITE(17,30)" RE(M1): ",M1N,M1NN,DEV(M1N,M1NN)
       ENDIF
       WRITE(17,30)" RE(M2): ",M2N,M2NN,DEV(M2N,M2NN)
       IF(M3FLAG.NE.0)THEN
        WRITE(17,30)" RE(M3): ",M3N,M3NN,DEV(M3N,M3NN)
       ENDIF
       WRITE(17,30)" RE(AU3): ",AQN,AQNN,DEV(AQN,AQNN)
       WRITE(17,30)" MQ3: ",MQN,MQNN,DEV(MQN,MQNN)
       WRITE(17,30)" RE(LAMBDA): ",LN,LNN,DEV(LN,LNN)
       WRITE(17,30)" RE(KAPPA): ",KN,KNN,DEV(KN,KNN)
       WRITE(17,30)" RE(MUEFF): ",MUN,MUNN,DEV(MUN,MUNN)
       IF(MOD(MAFLAG,3).EQ.0)THEN
        WRITE(17,30)" RE(ALAMBDA): ",ALN,ALNN,DEV(ALN,ALNN)
        WRITE(17,30)" MA: ",MAN,MANN
        WRITE(17,40)"(MA is not an input parameter)"
        IF(XIFN.NE.0d0 .OR. XIFNN.NE.0d0)
     .   WRITE(17,30)" RE(XIF): ",XIFN,XIFNN,DEV(XIFN,XIFNN)
       ELSEIF(MOD(MAFLAG,3).EQ.1)THEN
        WRITE(17,30)" RE(ALAMBDA): ",ALN,ALNN
        WRITE(17,40)"(RE(ALAMBDA) is not an input parameter)"
        WRITE(17,30)" MA: ",MAN,MANN,DEV(MAN,MANN)
        IF(XIFN.NE.0d0 .OR. XIFNN.NE.0d0)
     .   WRITE(17,30)"RE(XIF): ",XIFN,XIFNN,DEV(MAN,MANN)
       ELSE
        WRITE(17,30)" RE(ALAMBDA): ",ALN,ALNN,DEV(ALN,ALNN)
        WRITE(17,30)" MA: ",MAN,MANN,DEV(MAN,MANN)
        WRITE(17,30)" RE(XIF): ",XIFN,XIFNN
        WRITE(17,40)"(RE(XIF) is not an input parameter)"
       ENDIF
       IF(MAFLAG/3.EQ.0)THEN
        WRITE(17,30)" RE(AKAPPA): ",AKN,AKNN,DEV(AKN,AKNN)
        WRITE(17,30)" MP: ",MPN,MPNN
        WRITE(17,40)"(MP is not an input parameter)"
        IF(XISN.NE.0d0 .OR. XISNN.NE.0d0)
     .   WRITE(17,30)" RE(XIS): ",XISN,XISNN,DEV(XISN,XISNN)
       ELSEIF(MAFLAG/3.EQ.1)THEN
        WRITE(17,30)" RE(AKAPPA): ",AKN,AKNN
        WRITE(17,40)"(RE(AKAPPA) is not an input parameter)"
     ...
     .   WRITE(17,30)" RE(XIS): ",XISN,XISNN,DEV(XISN,XISNN)
       ELSE
        WRITE(17,30)" RE(AKAPPA): ",AKN,AKNN,DEV(AKN,AKNN)
        WRITE(17,30)" MP: ",MPN,MPNN,DEV(MPN,MPNN)
        WRITE(17,30)" RE(XIS): ",XISN,XISNN
        WRITE(17,40)"(RE(XIS) is not an input parameter)"
       ENDIF
       IF(MUPN.NE.0d0 .OR. MUPNN.NE.0d0)
     .  WRITE(17,30)" RE(MUP): ",MUPN,MUPNN,DEV(MUPN,MUPNN)
       IF(MSPN.NE.0d0 .OR. MSPNN.NE.0d0)
     . WRITE(17,30)" RE(MSP): ",MSPN,MSPNN,DEV(MSPN,MSPNN)
       IF(M3HN.NE.0d0 .OR. M3HNN.NE.0d0)
     .  WRITE(17,30)" RE(M3H): ",M3HN,M3HNN,DEV(M3HN,M3HNN)
       IF(M1IN.NE.0d0 .OR. M1INN.NE.0d0)
     .  WRITE(17,30)" IM(M1): ",M1IN,M1INN,DEV(M1IN,M1INN)
       IF(M2IN.NE.0d0 .OR. M2INN.NE.0d0)
     .  WRITE(17,30)" IM(M2): ",M2IN,M2INN,DEV(M2IN,M2INN)
       IF(M3IN.NE.0d0 .OR. M3INN.NE.0d0)
     .  WRITE(17,30)" IM(M3): ",M3IN,M3INN,DEV(M3IN,M3INN)
       IF(AU3IN.NE.0d0 .OR. AU3INN.NE.0d0)
     .  WRITE(17,30)" IM(AU3): ",AU3IN,AU3INN,DEV(AU3IN,AU3INN)
       IF(AD3IN.NE.0d0 .OR. AD3INN.NE.0d0)
     .  WRITE(17,30)" IM(AD3): ",AD3IN,AD3INN,DEV(AD3IN,AD3INN)
       IF(AE3IN.NE.0d0 .OR. AE3INN.NE.0d0)
     .  WRITE(17,30)" IM(AE3): ",AE3IN,AE3INN,DEV(AE3IN,AE3INN)
       IF(LIN.NE.0d0 .OR.LINN.NE.0d0)
     .  WRITE(17,30)" IM(LAMBDA): ",LIN,LINN,DEV(LIN,LINN)
       IF(KIN.NE.0d0 .OR. KINN.NE.0d0)
     .  WRITE(17,30)" IM(KAPPA): ",KIN,KINN,DEV(KIN,KINN)
       IF(XIFIN.NE.0d0 .OR. XIFINN.NE.0d0)
     .  WRITE(17,30)" IM(XIF): ",XIFIN,XIFINN,DEV(XIFIN,XIFINN)
       IF(XISIN.NE.0d0 .OR. XISINN.NE.0d0)THEN
        WRITE(17,30)" IM(XIS): ",XISIN,XISINN,DEV(XISIN,XISINN)
        IF(XISIFLAG.EQ.0)
     .   WRITE(17,40)"(IM(XIS) is not an input parameter)"
       ENDIF
       IF(MUPIN.NE.0d0 .OR. MUPINN.NE.0d0)
     .  WRITE(17,30)" IM(MUP): ",MUPIN,MUPINN,DEV(MUPIN,MUPINN)
       IF(MSPIN.NE.0d0 .OR. MSPINN.NE.0d0)
     .  WRITE(17,30)" IM(MSP): ",MSPIN,MSPINN,DEV(MSPIN,MSPINN)
       IF(M3HIN.NE.0d0 .OR. M3HINN.NE.0d0)
     .  WRITE(17,30)" IM(M3H): ",M3HIN,M3HINN,DEV(M3HIN,M3HINN)
       IF(ALIN.NE.0d0 .OR. ALINN.NE.0d0)THEN
        WRITE(17,30)" IM(AL): ",ALIN,ALINN,DEV(ALIN,ALINN)
        WRITE(17,40)"(IM(AL) is not an input parameter)"
       ENDIF
       IF(AKIN.NE.0d0 .OR. AKINN.NE.0d0)THEN
        WRITE(17,30)" IM(AK): ",AKIN,AKINN,DEV(AKIN,AKINN)
        WRITE(17,40)"(IM(AK) is not an input parameter)"
       ENDIF
