...
************************************************************************
      ...
      INTEGER HFLAG,UNCERTFLAG,GRFLAG,MWFLAG,CFLAG(6)
...
*   Beginning of the scan
 ...
 101    PAR(3)=TBCEN+MAX(DABS(TBCEN),TBMIN)*TBDEV*GAU(IDUM)
       ...
       IF(M2DEV.EQ.0d0)THEN
        PAR(21)=M2CEN
       ELSE
 102    PAR(21)=M2CEN+MAX(DABS(M2CEN),M2MIN)*M2DEV*GAU(IDUM)
        IF(PAR(21).GT.4d3)GOTO 102
       ENDIF
       ...
       ELSEIF(M1DEV.EQ.0d0)THEN
        PAR(20)=M1CEN
       ELSE
 103    PAR(20)=M1CEN+MAX(DABS(M1CEN),M1MIN)*M1DEV*GAU(IDUM)
        IF(PAR(20).GT.4d3)GOTO 103
       ENDIF
       ...
       ELSEIF(M3DEV.EQ.0d0)THEN
        PAR(22)=M3CEN
       ELSE
 104    PAR(22)=M3CEN+MAX(DABS(M3CEN),M3MIN)*M3DEV*GAU(IDUM)
        IF(PAR(22).GT.4d3)GOTO 104
       ENDIF
 ...
 105    PAR(1)=LCEN+MAX(DABS(LCEN),LMIN)*LDEV*GAU(IDUM)
       ...
       IF(MUDEV.EQ.0d0)THEN
        PAR(4)=MUCEN
       ELSE
 106    PAR(4)=MUCEN+MAX(DABS(MUCEN),MUMIN)*MUDEV*GAU(IDUM)
        IF(DABS(PAR(4)).GT.4d3)GOTO 106
       ENDIF
       ...
       IF(MOD(MAFLAG,3).EQ.0)THEN
        IF(ALDEV.EQ.0d0)THEN
         PAR(5)=ALCEN
        ELSE
 107     PAR(5)=ALCEN+MAX(DABS(ALCEN),ALMIN)*ALDEV*GAU(IDUM)
       ...
       IF(MAFLAG/3.EQ.0)THEN
        IF(AKDEV.EQ.0d0)THEN
         PAR(6)=AKCEN
        ELSE
 108     PAR(6)=AKCEN+MAX(DABS(AKCEN),AKMIN)*AKDEV*GAU(IDUM)
       ...
       IF(AU3DEV.EQ.0d0)THEN
        PAR(12)=AU3CEN
       ELSE
 109    PAR(12)=AU3CEN+MAX(DABS(AU3CEN),AU3MIN)*AU3DEV*GAU(IDUM)
        IF(DABS(PAR(12)).GT.4d3)GOTO 109
       ENDIF

       IF(AD3DEV.EQ.0d0)THEN
        PAR(13)=AD3CEN
       ELSE
 110    PAR(13)=AD3CEN+MAX(DABS(AD3CEN),AD3MIN)*AD3DEV*GAU(IDUM)
        IF(DABS(PAR(13)).GT.4d3)GOTO 110
       ENDIF

       IF(AE3DEV.EQ.0d0)THEN
        PAR(14)=AE3CEN
       ELSE
 111    PAR(14)=AE3CEN+MAX(DABS(AE3CEN),AE3MIN)*AE3DEV*GAU(IDUM)
        IF(DABS(PAR(14)).GT.4d3)GOTO 111
       ENDIF

       IF(AE2DEV.EQ.0d0)THEN
        PAR(25)=AE2CEN
       ELSE
 112    PAR(25)=AE2CEN+MAX(DABS(AE2CEN),AE2MIN)*AE2DEV*GAU(IDUM)
        IF(DABS(PAR(25)).GT.4d3)GOTO 112
       ENDIF

       IF(ML3DEV.EQ.0d0)THEN
        PAR(10)=ML3CEN**2
       ELSE
 113    PAR(10)=(ML3CEN+MAX(DABS(ML3CEN),ML3MIN)*ML3DEV*GAU(IDUM))**2
        IF(PAR(10).GT.16d6)GOTO 113
       ENDIF

       IF(ML2DEV.EQ.0d0)THEN
        PAR(18)=ML2CEN**2
       ELSE
 114    PAR(18)=(ML2CEN+MAX(DABS(ML2CEN),ML2MIN)*ML2DEV*GAU(IDUM))**2
        IF(PAR(18).GT.16d6)GOTO 114
       ENDIF

       IF(ME3DEV.EQ.0d0)THEN
        PAR(11)=ME3CEN**2
       ELSE
 115    PAR(11)=(ME3CEN+MAX(DABS(ME3CEN),ME3MIN)*ME3DEV*GAU(IDUM))**2
        IF(PAR(11).GT.16d6)GOTO 115
       ENDIF

       IF(ME2DEV.EQ.0d0)THEN
        PAR(19)=ME2CEN**2
       ELSE
 116    PAR(19)=(ME2CEN+MAX(DABS(ME2CEN),ME2MIN)*ME2DEV*GAU(IDUM))**2
        IF(PAR(19).GT.16d6)GOTO 116
       ENDIF

       IF(MQ3DEV.EQ.0d0)THEN
        PAR(7)=MQ3CEN**2
       ELSE
 117    PAR(7)=(MQ3CEN+MAX(DABS(MQ3CEN),MQ3MIN)*MQ3DEV*GAU(IDUM))**2
        IF(PAR(7).GT.16d6)GOTO 117
       ENDIF

       IF(MQ2DEV.EQ.0d0)THEN
        PAR(15)=MQ2CEN**2
       ELSE
 118    PAR(15)=(MQ2CEN+MAX(DABS(MQ2CEN),MQ2MIN)*MQ2DEV*GAU(IDUM))**2
        IF(PAR(15).GT.16d6)GOTO 118
       ENDIF

       IF(MU3DEV.EQ.0d0)THEN
        PAR(8)=MU3CEN**2
       ELSE
 119    PAR(8)=(MU3CEN+MAX(DABS(MU3CEN),MU3MIN)*MU3DEV*GAU(IDUM))**2
        IF(PAR(8).GT.16d6)GOTO 119
       ENDIF

       IF(MU2DEV.EQ.0d0)THEN
        PAR(16)=MU2CEN**2
       ELSE
 120    PAR(16)=(MU2CEN+MAX(DABS(MU2CEN),MU2MIN)*MU2DEV*GAU(IDUM))**2
        IF(PAR(16).GT.16d6)GOTO 120
       ENDIF

       IF(MD3DEV.EQ.0d0)THEN
        PAR(9)=MD3CEN**2
       ELSE
 121    PAR(9)=(MD3CEN+MAX(DABS(MD3CEN),MD3MIN)*MD3DEV*GAU(IDUM))**2
        IF(PAR(9).GT.16d6)GOTO 121
       ENDIF

       IF(MD2DEV.EQ.0d0)THEN
        PAR(17)=MD2CEN**2
       ELSE
 122    PAR(17)=(MD2CEN+MAX(DABS(MD2CEN),MD2MIN)*MD2DEV*GAU(IDUM))**2
        IF(PAR(17).GT.16d6)GOTO 122
       ENDIF
...
*   Recording of the results
 ...
 11   CALL MCMCSTEP(PAR,PROB,NPROB,IFAIL)
      IF(NTOT.EQ.1 .OR. ITOT.NE.1)CALL OUTPUT(PAR,PROB,IFAIL)
      IF(IFAIL.EQ.0)THEN
       TOT=TOT+1
       LN=MIN(PAR(1),LN)
       LNN=MAX(PAR(1),LNN)
...
*
     ...
     .      x,dNdx,EMIN,NBIN
      ...
      ENDDO
      DO I=1,2
       RES(IRES+5+2*I)=PMASS(1)
       RES(IRES+6+2*I)=PCOMP(1,2)**2
      ENDDO
      RES(IRES+11)=CMASS
      DO I=1,3
